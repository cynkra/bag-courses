...
res <- mutate(
  box_office,
  sum = box_office_us_canada + box_office_other,
  equal = sum == box_office_worldwide
)
all(res$equal, na.rm = TRUE)

# 2.  Flag movies in `box_office` that did better in the US and Canada than in the rest of the world
mutate(box_office, america_better = box_office_us_canada > box_office_other)

# 3.  Use `if_else()` and `median()` to create a column `budget_type` with values
#     `"low budget"` and `"high budget"`
mutate(box_office, budget_type = if_else(budget > median(budget, na.rm = TRUE), "high budget", "low budget"))

# 4.  Use `lag()` to create a column `days_since_previous_movie` in `pixar_films`
#     (The films are already sorted)
mutate(pixar_films, days_since_previous_movie = release_date - lag(release_date))
...
str_replace(x, "e", "r") # x goes first!
x %>%  str_replace("e", "r")
sub("e", "r", x) # x goes third!
x %>% sub("e", "r", .) # we need to provide a dot "." !
...
public_response %>%
  select(-cinema_score) %>%
  filter(if_all(rotten_tomatoes:critics_choice, ~ .x > 90))
