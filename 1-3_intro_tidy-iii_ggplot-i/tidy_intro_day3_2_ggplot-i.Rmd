...
We know a bit about it, but let's check `?geom_point` to check which aesthetics
are available we see: 
...
`aes()` is used to map variables to components.

**HOW** values are translated to a specific aesthetic
value (color, shape, coordinate) can be tweaked by `scale_*` functions, that
we'll see later. 
