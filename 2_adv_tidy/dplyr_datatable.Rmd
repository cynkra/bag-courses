...
???
looking at median runtimes, for small data, dplyr takes around 2-3 as much time, going up to almost 20x and even up to 40x for the case where we have more groups than rows; having a 10x speedups becomes very relevant as soon as the operation in question takes on the order of minutes
...
medication <- data.table(
  ...
  start_date = lubridate::ymd(c("2003-03-25", "2006-04-27", "2008-12-05",
                                "2004-01-03", "2005-09-18")),
  stop_date = lubridate::ymd(c("2006-04-02", "2012-02-03", "2011-05-03",
...
visit[, visit_date := sample(
  seq(lubridate::ymd("2003-01-01"), lubridate::ymd("2013-01-01"), 1),
  .N), by = id
]

visit[,some_hosp_measure := rnorm(.N, 10, 2)]
setkeyv(visit, c("id", "visit_date"))
...
visit[
  medication,
  mean(some_hosp_measure),
  on = .(id, visit_date > start_date, visit_date < stop_date),
...
visit %>%
  ...
  filter(visit_date > start_date,
         visit_date < stop_date) %>%
  group_by(id, medication) %>%
  summarise(V1 = mean(some_hosp_measure), .groups = "drop")
```
]
