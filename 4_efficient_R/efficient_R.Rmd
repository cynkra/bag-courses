...
👉️ Understand parallelization examples
...
Mainly for High-Performance-Computing (HPC) jobs. Requires often third-party software.
...
Be aware of the `%dopar%` operator.
...
Know where your data is written to!

Writing/compressing/serializing large datasets (MB, GB) will use the **write speed of the underlying disk**.
...
The mileage varies among protocols but it will usually never be percieved as "fast".
...
BUT: Avoid **big reoccurring read/write tasks on SSDs** if possible as this will quickly reduce a SSDs lifetime.
...
- {qs} package: https://github.com/traversc/qs
