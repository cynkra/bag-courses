...
### Let's take a look at this function and try to finish it.

<!-- FIXME: -->
<!-- We start by some slides suggesting good practice to follow for: -->
<!-- * argument names -->
<!-- * argument specification (as string or var name) -->
<!-- * error messages -->
<!-- * etc -->
<!-- Then we show what function we get when modifying f_make_age_groups -->

This function takes a data frame and adds a column containing the age group,
the names of the input column and the output columns are parameters, along with the breaks.
A boolean argument allow us to drop rows for which we don't have a category defined
...
When you need to bind rows or columns after a map* call, you might use the 
`map_dfr` or `map_dfc` variants

```{r, eval = FALSE}
    daten <- purrr::map2(names_list, breaks_list, .f = ~ {
      daten %>%
        f_make_age_groups(new_age_var_name = !!.x, age_breaks = .y) %>%
        dplyr::select(all_of(.x)) }) %>%
      dplyr::bind_cols(daten, .)
```

becomes :

```{r, eval = FALSE}
    daten <- purrr::map2_dfc(names_list, breaks_list, .f = ~ {
      daten %>%
        f_make_age_groups(new_age_var_name = !!.x, age_breaks = .y) %>%
        dplyr::select(all_of(.x)) })
```

Minor comment here but since we're overwriting a variable with something different, 
we could have changed the name of the variable.

# The `ws_PGM_372_make_dfs_rKTN_impfmonitoring.R` script

## More problems with summarize

When running the script we find that it builds datasets and shows a lot of grouping
warnings. Most of them can be solved in the script directly by using `.groups = "drop"`

Others are harder to debug because the error is nested into the function `vaccinations.add_sums()`

This function take as its inpout a grouped data frame and summarizes it.

The use of this function is always preceded by a group by.

I believe it is more intuitive to avoid as much as possible building a function
that takes a grouped data frame as an input, especially if this function
is not documented. It's much easier to pass the arguments to group_by to this
function directly.

FIXME : basic intro to environments ?

This script is building somne datasets (it used to also build the function `isoyear` but we've removed that part above)

To know which data sets are built without checking manually I restart can
source the first script in the global environment then source this script in a new environment and look at what was created.

save the variables that are in the environment.

```{r}
source(here::here("bag-code-example/scripts/ws_PGM_init.R"))
env <- new.env()
source(here::here("bag-code-example/scripts/ws_PGM_372_make_dfs_rKTN_impfmonitoring.R"), env)
eapply(env, class)
```

By glancing at this I can see if this script created only data or if I missed some
function definitions.

We could wrap this all in a function definition that returns a list,
but I suspect that we're not going to need all of those
...
Let's use it so we harmoinze the code and make it more understandable, even if it's a
couple lines longer :
...
We have the same issue with all the functions defined in `vaccinations.R`, this can be made much simpler
...
* values don't need to be `.dotted` since we don't have `...` (dotted names
  are used in the tidyverse to avoid conflict with the names the user might choose
  in `...`)

# simplify conditions

In the following we have a redundant condition :

```
((vacc_count == shots_until_complete_vacc & serie == 1) | (vacc_count == 1 & person_recovered_from_covid == TRUE)) & 
            person_age >= 12 & person_age < 16 & serie == 1
```

Can be written as 

```
(vacc_count == shots_until_complete_vacc | (vacc_count == 1 & person_recovered_from_covid == TRUE)) & 
            person_age >= 12 & person_age < 16 & serie == 1
```

It's not necessary to type `person_recovered_from_covid == TRUE` in conditions, `person_recovered_from_covid` is enough
...
sum(
  serie == 1 &
    ((vacc_count == shots_until_complete_vacc & vacc_count != 2) | 
       vacc_count == 1 & person_recovered_from_covid |
       vacc_count == shots_until_complete_vacc & vacc_count == 2 & !person_recovered_from_covid
    ), 
  na.rm = TRUE
)
```
